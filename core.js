...
function addCoins(n){localStorage.setItem("allen_coins", getCoins()+n);}
...
function playHint(word){const saved=localStorage.getItem("hint_"+word);if(!saved)return alert("⚠️ 尚無提示錄音");new Audio(saved).play();}
function recordHint(word){
  const btn=event.target;const status=$("#recordingStatus");
  ...
  navigator.mediaDevices.getUserMedia({audio:true}).then(stream=>{
    ...
    recorder.onstop=()=>{
      ...
      reader.onloadend=()=>{
        ...
        status.textContent="✅ 錄音已儲存！";setTimeout(()=>status.textContent="",2000);
      };reader.readAsDataURL(blob);
    };
    recorder.start();btn.textContent="⏹️ 停止錄音";btn.dataset.recording="true";
    timeLeft=10;status.textContent=`🎙️ 錄音中… (${timeLeft})`;
    countdownTimer=setInterval(()=>{
      timeLeft--;status.textContent=`🎙️ 錄音中… (${timeLeft})`;
      if(timeLeft<=0)stopRecording(word,btn,status);
    },1000);
  }).catch(()=>alert("無法使用錄音功能"));
}
function stopRecording(word,btn,status){clearInterval(countdownTimer);if(recorder&&recorder.state==="recording")recorder.stop();btn.textContent="🎙️ 錄音提示";btn.dataset.recording="false";status.textContent="";}
...
function startStars(){
  ...
  function draw(){
    ...
    stars.forEach(star=>{ctx.globalAlpha=Math.random()*0.5+0.3;ctx.beginPath();ctx.arc(star.x,star.y,star.r,0,Math.PI*2);ctx.fill();star.y+=star.s;if(star.y>h)star.y=0;});
